...
const PRCreateJob = () => {
  ...
  useEffect(() => {
    if (!user || (user.role !== 'pr' && user.role !== 'placement_representative')) {
      navigate('/login');
      return;
    }
  }, [user, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...formData,
        type: formData.type, // Send as 'type'
        jobType: formData.type, // Also send as 'jobType' for compatibility
        ctc: parseFloat(formData.ctc),
        skills: formData.skills.split(',').map(s => s.trim()).filter(s => s),
        rounds: formData.rounds.split(',').map(s => s.trim()).filter(s => s),
        eligibility: {
      ...
      console.log('Deadline value:', formData.deadline);

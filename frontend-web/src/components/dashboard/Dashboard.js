...
const Dashboard = () => {
  const { user } = useAuth();
  const { fetchJobDrives } = useJobDrives();
  ...
  // Add error logging
  Logger.debug('Dashboard rendering, user:', user);

  useEffect(() => {
    Logger.debug('Dashboard useEffect running');
    fetchDashboardData();
    loadJobDrives();
  }, []);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      Logger.debug('Dashboard stats response:', response.data);
  ...
  const loadJobDrives = async () => {
    try {
      Logger.debug('Fetching job drives...');
      const result = await fetchJobDrives();
      Logger.debug('Job drives response:', result);
      setJobDrives(result.jobDrives || result || []);
    } catch (error) {
  ...
  const handleCSVUpload = async (event) => {
    ...
    try {
      ...
      Logger.debug('CSV Upload Response:', response.data);
      event.target.value = '';
    } catch (error) {

...
const StudentDashboard = () => {
  ...
  const { fetchJobDrives, applyToJobDrive } = useJobDrives();
  ...
  const fetchEligibleDrives = useCallback(async () => {
    ...
    try {
      ...
      // Use the Redux fetchJobDrives function
      const result = await fetchJobDrives();
      console.log("Drives response:", result);
      const drives = result.drives || result || [];
      setEligibleDrives(drives);

      // Also fetch all drives for stats
      const allDrivesResult = await fetchJobDrives();
      const allDrivesData = allDrivesResult.jobDrives || allDrivesResult || [];

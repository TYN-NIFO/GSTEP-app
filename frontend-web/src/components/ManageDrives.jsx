...
const ManageDrives = () => {
  ...
  const fetchPlacedStudents = async (jobDriveId, companyName, drive) => {
    try {
      // First try to use the placed students data from the drive object itself
      ...
      // If no placed students in drive object, try to fetch from API
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5000/api/job-drives/${jobDriveId}/placed-students`, {
        method: 'GET',
        headers: {
      ...
      if (response.ok) {
        ...
        setSelectedPlacedStudents(data.placedStudents || []);
        setSelectedPlacedDriveName(companyName);
        setShowPlacedStudentsModal(true);
      } else {
        // If API fails, show empty modal
        console.log('API response not ok, showing empty placed students list');
      ...
      // Show empty modal instead of error toast
      setSelectedPlacedStudents([]);
      setSelectedPlacedDriveName(companyName);
      setShowPlacedStudentsModal(true);
    }
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex space-x-2 ml-4">
                        ...
                        {isDriveEnded(drive) ? (
                          ...
                          >
                            List of Placed Students ({drive.placedStudents?.length || 0})
                          </button>
                        ) : (
                          <button
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                      <thead className="bg-gray-50">
                        <tr>
                          ...
                          <th className="px-4 py-3 text-left text-sm font-medium text-gray-500 uppercase">Mobile No</th>

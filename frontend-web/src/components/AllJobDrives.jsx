...
const AllJobDrives = () => {
  ...
  // Function to check if drive has ended (considering both date and time)
  const isDriveEnded = (drive) => {
    if (!drive.date) return false;
    
    const driveDateTime = new Date(drive.date);
    // If drive has a specific time, use it; otherwise assume end of day
    if (drive.time) {
      const [hours, minutes] = drive.time.split(':');
      driveDateTime.setHours(parseInt(hours), parseInt(minutes), 0, 0);
    } else {
      // If no time specified, consider drive ended at end of the day
      driveDateTime.setHours(23, 59, 59, 999);
    }
    
    const currentDateTime = new Date();
    return currentDateTime > driveDateTime;
  };
  ...
  const fetchAllJobDrives = async () => {
    try {
      ...
      // If we have a selected drive for view, update it with fresh data
      if (selectedDriveForView) {
        const updatedDrive = response.data.jobDrives.find(drive => drive._id === selectedDriveForView._id);
        if (updatedDrive) {
          setSelectedDriveForView(updatedDrive);
        }
      }
  ...
  const getFilteredDrives = () => {
    ...
    return drives.filter(drive => {
      ...
      // Then apply other filters using the new logic
                      ...
                      {user?.role === 'placement_representative' && driveEnded && (
                        <>
                          ...
                          {drive.placedStudents && drive.placedStudents.length > 0 && (
                            ...
                            >
                              View Placed Students ({drive.placedStudents.length})
                            </button>
                          )}
                        </>
                      )}

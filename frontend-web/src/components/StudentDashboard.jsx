...
const StudentDashboard = () => {
  ...
  const hasApplied = (drive) => {
    ...
    return drive.applications.some((app) => {
      ...
      return app === userId || app === user?.id || app === user?._id;
    });
  };
  ...
  const isEligibleForDrive = (drive) => {
    ...
    const minCGPA = parseFloat(drive.eligibility?.minCGPA || drive.eligibility?.cgpa) || 0;
    const maxBacklogs = parseInt(drive.eligibility?.maxBacklogs) || 0;
    const userBacklogs = parseInt(user.profile.backlogs) || 0;

    // Check basic eligibility
    if (userCGPA < minCGPA) return false;
    if (userBacklogs > maxBacklogs) return false;

    // Check department eligibility
    if (drive.eligibility?.allowedDepartments?.length > 0) {
      if (!drive.eligibility.allowedDepartments.includes(user.profile.department)) {
        return false;
      }
    }
    ...
    if (drive.eligibility?.allowedBatches?.length > 0) {
      if (!drive.eligibility.allowedBatches.includes(user.profile.batch)) {
        return false;
      }
    }

    // Check placement status eligibility
    const isUserPlaced = user.profile.isPlaced || false;

...
import { useJobDrives } from '../../redux/customHooks/useJobDrives';
import toast from 'react-hot-toast';
import Logger from '../../utils/logger';

const JobDrives = () => {
  const { user } = useAuth();
  const { jobDrives, loading, fetchJobDrives, applyToJobDrive, withdrawApplication } = useJobDrives();
  
  // Debug student data immediately
  Logger.debug('=== JOBDRIVES COMPONENT LOADED ===');
  Logger.debug('User role:', user?.role);
  Logger.debug('User profile:', user?.profile);
  
  if (user?.role === 'student' && user?.profile) {
    Logger.debug('=== STUDENT PROFILE DETAILS ===');
    Logger.debug('CGPA:', user.profile.cgpa, typeof user.profile.cgpa);
    Logger.debug('Department:', user.profile.department);
    Logger.debug('Batch:', user.profile.batch);
    Logger.debug('Current Backlogs:', user.profile.currentBacklogs);
    Logger.debug('Backlogs:', user.profile.backlogs);
    Logger.debug('Is Placed:', user.profile.isPlaced);
    Logger.debug('Placement Status:', user.profile.placementStatus);
  }
  ...
  useEffect(() => {
    Logger.debug('JobDrives component mounted');
    ...
    if (user?.role === 'student' && user?.profile) {
      Logger.debug('=== STUDENT PROFILE DEBUG ===');
      Logger.debug('CGPA:', user.profile.cgpa);
      Logger.debug('Department:', user.profile.department);
      Logger.debug('Batch:', user.profile.batch);
      Logger.debug('Current Backlogs:', user.profile.currentBacklogs);
      Logger.debug('Backlogs:', user.profile.backlogs);
      Logger.debug('Is Placed:', user.profile.isPlaced);
      Logger.debug('Placement Status:', user.profile.placementStatus);
      Logger.debug('=== END STUDENT PROFILE ===');
    }
  ...
  useEffect(() => {
    if (jobDrives.length > 0 && user) {
      Logger.debug('=== DEBUG DATA ===');
      Logger.debug('User object:', user);
      Logger.debug('User ID:', user.id);
      Logger.debug('User _id:', user._id);
      Logger.debug('First drive applications:', jobDrives[0]?.applications);
      jobDrives.forEach((drive, index) => {
        Logger.debug(`Drive ${index} (${drive.companyName}) applications:`, drive.applications);
      });
  ...
  useEffect(() => {
    if (jobDrives.length > 0 && user) {
      ...
      jobDrives.forEach(drive => {
        Logger.debug(`Drive: ${drive.companyName}`);
        Logger.debug('Applications:', drive.applications);
        Logger.debug('User ID:', user?.id || user?._id);
        Logger.debug('Has Applied:', hasApplied(drive));
        Logger.debug('---');
      });
    }
  }, [jobDrives, user]);

  // fetchJobDrives is now handled by Redux useJobDrives hook

  const handleApply = async (driveId) => {
    try {
      Logger.debug('Applying to drive:', driveId);
      Logger.debug('User:', user);
      
      await applyToJobDrive(driveId);
      toast.success('Application submitted successfully!');
      
    } catch (error) {
      console.error('Apply error:', error);
      toast.error(error.message || 'Failed to apply to job drive');
    }
  };
  ...
  const getFilteredDrives = () => {
    ...
    console.log('Total drives:', jobDrives.length);
    console.log('Current filter:', filter);
    console.log('User role:', user?.role);
    
    let filtered = jobDrives.filter(drive => {
                    ...
                    <div className="flex flex-col space-y-2 ml-4">
                      ...
                      {(user?.role === 'placement_representative' || user?.role === 'pr') && (
                        <div className="space-y-2">
                          ...
                          {!driveEnded && isEligible && !hasApplied(drive) && (
                            <button
                              onClick={() => handleApply(drive._id)}
